...
const ComboboxItem: Component<{
  ...
  return (
    <div
      ...
      class="relative flex cursor-pointer select-none items-center rounded-lg px-2.5 py-1.5 text-sm outline-none
        hover:bg-white/[0.06] transition-colors duration-100"
      role="option"
    >
      {props.text}
      <Show when={props.selected}>
        <svg width="15" height="15" viewBox="0 0 15 15" fill="none" class="ml-auto h-4 w-4 text-emerald-400">
  ...
  return (
    <div class="relative min-w-48">
      <button
        ...
        class="relative inline-flex items-center whitespace-nowrap rounded-lg text-sm font-medium
          transition-all duration-200 focus-visible:outline-none focus-visible:ring-1 focus-visible:ring-ring
          disabled:pointer-events-none disabled:opacity-50 border border-white/[0.1] bg-white/[0.03]
          shadow-sm hover:bg-white/[0.06] hover:border-white/[0.15] h-9 px-4 py-2 w-full justify-between"
        role="combobox"
      ...
      >
        ...
        <svg
          ...
          viewBox="0 0 15 15"
          fill="none"
          class="ml-2 h-4 w-4 shrink-0 opacity-40"
        >
          <path
      ...
      <Show when={opened()}>
        <div class="absolute top-full left-0 z-50 mt-1.5 w-full dropdown-enter">
          <div
            role="dialog"
            class="rounded-xl border border-white/[0.08] bg-[hsl(var(--popover))] text-popover-foreground shadow-xl shadow-black/30 outline-none"
            tabindex="-1"
          >
